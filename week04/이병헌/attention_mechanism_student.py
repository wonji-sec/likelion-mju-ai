...
Original file is located at
    https://colab.research.google.com/drive/1lZl0hCqKW4aR73C2FhdgnwnXlCqSqNKK
...
def get_attention_scores(inputs, query_index):
    ...
    attn_scores = torch.empty(inputs.shape[0]) # 입력에 들어온 토큰 개수(input.shape)만큼의 크기를 가진 빈 텐서를 만든다.

    # TODO 3. 모든 입력 벡터와 query의 dot product를 계산하세요.
    for i, x_i in enumerate(inputs):
        attn_scores[i] = torch.dot(x_i, query) # 어떤 단어와 연관성이 높은지 for문을 돌려서 각 단어의 dot product 계산
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_in, d_out, context_length, dropout, num_heads, qkv_bias=False):
        super().__init__()
        # d_out이 헤드 수로 나누어 떨어지는지 확인
        assert d_out % num_heads == 0, "d_out은 num_heads로 나누어 떨어져야 합니다."

        self.d_out = d_out
        self.num_heads = num_heads
        self.head_dim = d_out // num_heads # 각 헤드의 크기

        # 1. Q, K, V를 만들기 위한 선형 층
        self.W_query = nn.Linear(d_in, d_out, bias=qkv_bias)
        self.W_key = nn.Linear(d_in, d_out, bias=qkv_bias)
        self.W_value = nn.Linear(d_in, d_out, bias=qkv_bias)
    ...
    def forward(self, x):
        ...
        # head 분리: [b, num_tokens, num_heads, head_dim]
        keys = keys.view(b, num_tokens, self.num_heads, self.head_dim)
        queries = queries.view(b, num_tokens, self.num_heads, self.head_dim)
        values = values.view(b, num_tokens, self.num_heads, self.head_dim)
        ...
        # 미래 단어 위치를 -inf로 채워 softmax 결과가 0이 되게 함
        mask_bool = self.mask.bool()[:num_tokens, :num_tokens]
        attn_scores.masked_fill_(mask_bool, -torch.inf)
        ...
        # 벡터 계산 및 헤드 다시 합치기
        # 다시 원래 크기인 [b, num_tokens, d_out]으로 복원
        context_vec = (attn_weights @ values).transpose(1, 2)
        context_vec = context_vec.contiguous().view(b, num_tokens, self.d_out)
