...
def show_status(name: str, weight: int, hp: int, hunger: float) -> None: # None은 반환값이 없다는 뜻
    print(f"이름: {name}")
    print(f"체중: {weight}")
    print(f"체력: {hp}")
    print(f"배고픔: {hunger}")
...
def create_character():
    ...
    show_status(name=name, weight=weight, hp=hp, hunger=hunger) # 입력값을 순서대로 안넣었을때는 키워드 인자 방식으로 넣어주면 된다. (순서 상관없이 입력 가능)
